...
def part2(data: list[str]):
    """Solve part 2."""
    zeros: list[tuple[int, int]] = [] # we want to store the positions (x, y) of all the zeros
    nines: dict[tuple[int, int], list[tuple[int, int]]] = {} # we will store all the nines we visited successfully here
    for x, line in enumerate(data):
        for y, digit in enumerate(line):
    ...
    for zero in zeros:
        perform_walk2(data, zero, nines, zero)
...
def perform_walk2(data: list[str], target: tuple[int, int], nines: dict[tuple[int, int], list[tuple[int, int]]], trailhead: tuple[int, int]) -> int:
    # we want to find all our neighbours (up, down, right, left) which is 1 more than us
    target_value = int(data[target[0]][target[1]])
    ...
    for neighbor in neighbors:
        perform_walk2(data, neighbor, nines, trailhead)
